<?php
	...
	if($_GET['action'] == 'updateStatus' && isset($_SESSION['logged_in'])) {
	...
	} else if($_GET['action'] == 'updateStatus') exit_and_echo($return, false, "User not logged in");
	
	if($_GET['action'] == 'inviteToGame') {
		if(!isset($_GET['playerID'])) exit_and_echo($return, false, "Missing inviteID");
		else {		
			$database->query(
				sprintf("INSERT INTO `games` (`id`, `status`, `first_player`, `second_player`, `map`, `active_time`, `round`) VALUES (NULL, '0', '".$_SESSION['logged_in']."', '%s', '', '".time()."', 'white');",
	...
	if($_GET['action'] == 'acceptInvite') {
		if(!isset($_GET['playerID'])) exit_and_echo($return, false, "Missing inviteID");
			...
			$database->query(sprintf(
				"UPDATE `games` set `checkerboard` = ".'"'.$chessboard->showMap_().'"'." WHERE `games`.`second_player` = ".$_SESSION['logged_in']." AND `games`.`first_player` = %s",
				mysqli_real_escape_string($database, $_GET['playerID'])));
